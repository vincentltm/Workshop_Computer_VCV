...
public:
  ...
  The Run method starts audio processing, calling ProcessSample using an
  interrupt. Run is a blocking function (it never returns)
  */
  void Run() {
...
protected:
  class NotchFilter {
  public:
    NotchFilter() { mix1 = mix2 = mixf1 = mixf2 = 0; }
  ...
  private:
    ...
    static constexpr int32_t ooa0 = 16302,
                             a2oa0 = 16221; // Q = 100, very narrow notch
  };
  ...
  /// Optional background loop for USB tasks, MIDI RX, etc. Called from
  /// AudioWorker
  ...
  /// Read switch position
  bool __not_in_flash_func(SwitchChanged)() {
    return switchVal != lastSwitchVal;
  }
  ...
  bool __not_in_flash_func(PulseInRisingEdge)(int i) {
    return pulse[i] && !last_pulse[i];
  }
  /// Return true for one sample on pulse falling edge
  bool __not_in_flash_func(PulseInFallingEdge)(int i) {
    return !pulse[i] && last_pulse[i];
  }
  ...
  bool __not_in_flash_func(PulseIn1RisingEdge)() {
    return pulse[0] && !last_pulse[0];
  }
  /// Return true for one sample on pulse 1 falling edge
  bool __not_in_flash_func(PulseIn1FallingEdge)() {
    return !pulse[0] && last_pulse[0];
  }
  ...
  bool __not_in_flash_func(PulseIn2FallingEdge)() {
    return !pulse[1] && last_pulse[1];
  }
  /// Return true for one sample on pulse 2 rising edge
  bool __not_in_flash_func(PulseIn2RisingEdge)() {
    return pulse[1] && !last_pulse[1];
  }
  ...
  HardwareVersion_t HardwareVersion() const { return hw; }

  /// Return ID number unique to flash card
  uint64_t UniqueCardID() const { return uniqueID; }

  /// Return true iff CV outputs are calibrated.
  /// Returns false if using default calibration values.
  bool CVOutsCalibrated() const { return cvOutsCalibrated; }
...
private:
  ...
  static void AudioCallback() { thisptr->BufferFull(); }
...
#define COMPUTERCARD_SAMPLE_RATE_DIV 2
#endif
  adc_set_clkdiv(124 * COMPUTERCARD_SAMPLE_RATE_DIV +
                 COMPUTERCARD_SAMPLE_RATE_DIV - 1);
  ...
  while (1) {
    ...
    if (runADCMode == RUN_ADC_MODE_REQUEST_ADC_RESTART) {
      ...
      // We can't remove the PWM IRQ from within the ADC IRQ callback, so we do
      // it here instead.
      irq_set_enabled(PWM_IRQ_WRAP, false);
      pwm_clear_irq(
          pwm_gpio_to_slice_num(CV_OUT_1)); // reset CV PWM interrupt flag
      irq_remove_handler(PWM_IRQ_WRAP, ComputerCard::OnCVPWMWrap);
      break;
    }
    // Run background tasks (USB MIDI, MIDI RX, etc.) between ISR callbacks
    if (thisptr)
      thisptr->BackgroundLoop();
  }
}

void ComputerCard::Abort() { runADCMode = RUN_ADC_MODE_REQUEST_ADC_STOP; }
...
void __not_in_flash_func(ComputerCard::CorrectADCDNL)(uint16_t &value) const {
  ...
  value += (adc512 >> 10) << 3;
  value = uint32_t(value * 520349) >>
          19; // Multiply by factor that maps 0-4095 input into 0-4095 output
}
...
void __not_in_flash_func(ComputerCard::BufferFull)() {
  static int startupCounter =
      8; // Decreases by 1 each sample, can do startup things when nonzero.
  ...
  // ADC round-robin resync: if the FIFO overflowed, we must clear the flag
  // and reset the ADC to restore channel alignment.
  ...
  adcInR =
      -(((ADC_Buffer[cpuPhase][0] + ADC_Buffer[cpuPhase][4]) - 0x1000) >> 1);
  adcInL =
      -(((ADC_Buffer[cpuPhase][1] + ADC_Buffer[cpuPhase][5]) - 0x1000) >> 1);
  ...
  if (useNormProbe) {
    ...
    if (norm_probe_count == 14 || norm_probe_count == 15) {
      plug_state[2 + cvi] =
          (plug_state[2 + cvi] << 1) + (ADC_Buffer[cpuPhase][7] < 1800);
    }

    // Audio and pulse measured every sample at 48kHz
    if (norm_probe_count == 15) {
      plug_state[Input::Audio1] =
          (plug_state[Input::Audio1] << 1) + (ADC_Buffer[cpuPhase][5] < 1800);
      plug_state[Input::Audio2] =
          (plug_state[Input::Audio2] << 1) + (ADC_Buffer[cpuPhase][4] < 1800);
    ...
    // Force disconnected values to zero, rather than the normalisation probe
    // garbage
...
ComputerCard::HardwareVersion_t ComputerCard::ProbeHardwareVersion() {
  ...
  uint8_t pd = gpio_get(BOARD_ID_0) | (gpio_get(BOARD_ID_1) << 2) |
               (gpio_get(BOARD_ID_2) << 4);
  ...
  uint8_t pu = (gpio_get(BOARD_ID_0) << 1) | (gpio_get(BOARD_ID_1) << 3) |
               (gpio_get(BOARD_ID_2) << 5);
...
ComputerCard::ComputerCard() {
  ...
  gpio_pull_up(
      PULSE_1_INPUT); // NB Needs pullup to activate transistor on inputs

  gpio_init(PULSE_2_INPUT);
  gpio_set_dir(PULSE_2_INPUT, GPIO_IN);
  gpio_pull_up(
      PULSE_2_INPUT); // NB: Needs pullup to activate transistor on inputs
  ...
  // We set up the PWM here, and add the IRQ for sigma-delta later one Run() is
  // called
  ...
  // now create PWM config struct
  {
    ...
    // now set this PWM config to apply to the two outputs
    // NB: CV_A and CV_B share the same PWM slice, which means that they share a
    // PWM config They have separate 'gpio_level's (output compare unit) though,
    // so they can have different PWM on-times
    pwm_init(pwm_gpio_to_slice_num(CV_OUT_1), &config,
             true); // Slice 1, channel A
    pwm_init(pwm_gpio_to_slice_num(CV_OUT_2), &config,
             true); // slice 1 channel B (redundant to set up again)
  }
...
uint8_t ComputerCard::ReadByteFromEEPROM(unsigned int eeAddress, bool &failed) {
  ...
  if (i2c_write_timeout_us(i2c0, deviceAddress, &addr_low_byte, 1, false,
                           10000) <= 0) {
    failed = true;
    return 0;
  }
...
int ComputerCard::ReadEEPROM() {
  ...
  // Read magic number
  // Failure here could occur if I2C failed, or if incorrect/no magic number
  // stored in EEPROM
  ...
  uint16_t foundCRC =
      ((uint16_t)buf[EEPROM_ADDR_CRC_H] << 8) | buf[EEPROM_ADDR_CRC_L];
...
void ComputerCard::CalcCalCoeffs(int channel) {
  ...
  calCoeffs[channel].mi =
      int32_t(calCoeffs[channel].m * 1.333333333333333f + 0.5f);
...
/// Converts voltage in millivolts to corresponding 19-bit sigma-delta PWM DAC
/// value Returns true if requested voltage is outside of full range of DAC
/// values millivolts should be in range -6000 to 6000. Accuracy is dependent,
/// of course, on the calibration coefficients
uint32_t ComputerCard::MillivoltsToDAC(int millivolts, int channel,
                                       bool &limited) {
  limited = false;
  int32_t dacValue =
      ((((calCoeffs[channel].mi * millivolts) >> 9) * 1573) >> 12) +
      calCoeffs[channel].bi;
  if (dacValue > 524287) {
    dacValue = 524287;
    limited = true;
  }
