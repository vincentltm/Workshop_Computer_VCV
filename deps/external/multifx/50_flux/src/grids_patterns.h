#ifndef GRIDS_PATTERNS_H
#define GRIDS_PATTERNS_H

#include <stdint.h>

namespace Grids {

// MI Grids pattern data extracted from pichenettes/eurorack/grids/resources.cc
// Each node is 96 bytes: 32 steps * 3 channels (BD, SD, HH)
// Interleaving: step0_BD, step0_SD, step0_HH, step1_BD...
...
inline uint8_t ReadDrumMap(uint8_t step, uint8_t instrument, uint8_t x, uint8_t y) {
  uint8_t i = x >> 5; // 0..3
  uint8_t j = y >> 5; // 0..3
  if (i > 3) i = 3;
  if (j > 3) j = 3;
  ...
  uint8_t x_frac = (x & 0x1F) << 3; // 0..248
  uint8_t y_frac = (y & 0x1F) << 3; // 0..248
