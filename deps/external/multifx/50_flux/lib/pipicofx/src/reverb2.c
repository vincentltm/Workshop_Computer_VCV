...
{
    initReverb2Extended(reverbData, 0);
}

void initReverb2Extended(Reverb2Type*reverbData, int32_t offset)
{
    int16_t * delayMemoryPointer = getDelayMemoryPointer() + offset;
...
int16_t reverb2ProcessSampleWet(int16_t sampleIn,Reverb2Type*reverbData)
{
    int32_t reverbSignal=0;
    volatile uint32_t * audioStatePtr = getAudioStatePtr();
    int32_t processSignal = (sampleIn >> 1) + ((reverbData->decay*reverbData->outs[3])>> 15);
    ...
    return (int16_t)clip(reverbSignal,audioStatePtr);
}