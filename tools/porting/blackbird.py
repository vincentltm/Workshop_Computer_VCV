...
def post_process(content, src_rel):
    if src_rel == "main.cpp":
        ...
        content = re.sub(
            r'extern\s+"C"\s*\{\s*extern\s+const\s+unsigned\s+char\s+clock\[\];\s*extern\s+const\s+unsigned\s+int\s+clock_len;\s*\}',
        ...
        content = content.replace('extern extern "C"', 'extern "C"')
