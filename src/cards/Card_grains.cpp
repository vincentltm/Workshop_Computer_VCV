...
// Card: grains (manual rewrite - do not regenerate)
  ...
  void BackgroundLoop() override {
    if (flashAction >= 0) {
      ...
      while (!core1_is_paused) {
        (void)0; // FIFO drain handled by core1 thread
      }
      while (g_fifo_0_to_1.size() > 0)
        g_fifo_0_to_1.pop();
...
void __not_in_flash_func(core1_worker)() {
  ...
  while (1) {
    ...
    while (g_fifo_0_to_1.size() == 0 && !g_cancellation_requested.load(std::memory_order_relaxed)) { std::this_thread::sleep_for(std::chrono::microseconds(10)); }
    if (g_cancellation_requested.load(std::memory_order_relaxed)) break;
    uint32_t trig = g_fifo_0_to_1.pop();
    ...
    if (isTape) {
      ...
      if (buf && bS > 0) {
        ...
        g_fifo_1_to_0.push((uint32_t)out);
        g_fifo_1_to_0.push((uint32_t)out);
        g_fifo_1_to_0.push((uint32_t)((int32_t)(smoothedDelta * 1024.0))); // Pass scaled speed for CV2 output
      } else {
    ...
    if (gptr->inFlashMenu) {
      ...
      g_fifo_1_to_0.push((uint32_t)sL);
      g_fifo_1_to_0.push((uint32_t)sR);
      g_fifo_1_to_0.push(0);
      continue;
    }
    if (trig & 2)
    ...
    g_fifo_1_to_0.push((uint32_t)sL);
    g_fifo_1_to_0.push((uint32_t)sR);
    g_fifo_1_to_0.push((uint32_t)eS);
  }
}
int main() {
