...
struct WorkshopComputer : Module, IGridConsumer, IComputerModule {
    ...
    float get_switch_z_val() const override { return params[SWITCH_Z_PARAM].value; }
    ...
    void update_metadata_labels() {
        ...
        debug_log << "WorkshopComputer::update_metadata_labels called: card_id=" << card_id << " z_val=" << z_val << "\n";
        debug_log.close();
        if (z_val < 0) z_val = 0;
        if (z_val > 2) z_val = 2;
        ...
        int input_mapping[6] = {
            AUDIO_1_INPUT, AUDIO_2_INPUT, CV_1_INPUT, CV_2_INPUT, PULSE_1_INPUT, PULSE_2_INPUT
        };
        int output_mapping[6] = {
            AUDIO_1_OUTPUT, AUDIO_2_OUTPUT, CV_1_OUTPUT, CV_2_OUTPUT, PULSE_1_OUTPUT, PULSE_2_OUTPUT
        };
        ...
        int knob_mapping[3] = {
            KNOB_MAIN_PARAM, KNOB_X_PARAM, KNOB_Y_PARAM
        };
    ...
    void run_integration_tests() {
        int original_card = active_card_idx;
        int passed_count = 0;
        int failed_count = 0;
...
struct WorkshopComputerWidget : ModuleWidget {
    ...
    void step() override {
        ModuleWidget::step();
        WorkshopComputer* m = dynamic_cast<WorkshopComputer*>(module);
        if (m) {
